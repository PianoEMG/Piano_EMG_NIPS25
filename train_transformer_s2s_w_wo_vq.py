...
def get_configurations():
    ...
    opt.en_channels_emg = [128, 256, opt.dim_vq_latent]
    opt.de_channels_emg = [opt.dim_vq_latent, 256, 128, 6]
    opt.en_channels_keystroke = [128, 256, opt.dim_vq_latent]
    opt.de_channels_keystroke = [opt.dim_vq_latent, 256, 128, 88]
...
def load_models(opt):
    vq_e_encoder = VQEncoderV3(input_size=6, channels=opt.en_channels_emg, n_down=3)
    vq_e_decoder = VQDecoderV3(input_size=opt.dim_vq_latent, channels=opt.de_channels_emg, n_resblk=2, n_up=3)
    e_quantizer = Quantizer(opt.codebook_size, opt.dim_vq_latent, opt.lambda_beta)

    vq_k_encoder = VQEncoderV3(input_size=88, channels=opt.en_channels_keystroke, n_down=3)
    vq_k_decoder = VQDecoderV3(input_size=opt.dim_vq_latent, channels=opt.de_channels_keystroke, n_resblk=2, n_up=3)
    k_quantizer = Quantizer(opt.codebook_size, opt.dim_vq_latent, opt.lambda_beta)

    vq_checkpoint_e = torch.load('../../Piano_EMG_NIPS25_checkpoints/VQ_Model/emg_VQ/2025-04-04_18-02-21 128x256/finest.tar',
                            map_location='cuda')
    vq_e_encoder.load_state_dict(vq_checkpoint_e['vq_encoder'])    
    vq_e_decoder.load_state_dict(vq_checkpoint_e['vq_decoder'])
    e_quantizer.load_state_dict(vq_checkpoint_e['quantizer'])
    vq_e_encoder.to(device)
    vq_e_decoder.to(device)
    e_quantizer.to(device)

    vq_checkpoint_k = torch.load('../../Piano_EMG_NIPS25_checkpoints/VQ_Model/keystroke_VQ/2025-04-07_16-19-14 128x256/finest.tar',
                            map_location='cuda')
    vq_k_encoder.load_state_dict(vq_checkpoint_k['vq_encoder'])    
    vq_k_decoder.load_state_dict(vq_checkpoint_k['vq_decoder'])
    k_quantizer.load_state_dict(vq_checkpoint_k['quantizer'])
    vq_k_encoder.to(device)
    vq_k_decoder.to(device)
    k_quantizer.to(device)
    return vq_e_encoder, vq_e_decoder, e_quantizer, vq_k_encoder, vq_k_decoder, k_quantizer


def train_and_val(model, train_loader, val_loader, optimizer, loss_fn, scheduler, opt, epoch, device):
    if opt.use_vq:
        vq_e_encoder, vq_e_decoder, e_quantizer, vq_k_encoder, vq_k_decoder, k_quantizer = load_models(opt)
        vq_e_encoder.eval()
        vq_e_decoder.eval()
        e_quantizer.eval()
        vq_k_encoder.eval()
    ...
    for i, data in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}/{opt.epoch}", unit="batch")):
        ...
        if opt.use_vq:
            k_latents = vq_k_encoder(data['keystroke'])
            embedding_loss_k, vq_latents_k, _, perplexity_k = k_quantizer(k_latents)
            e_latents = vq_e_encoder(data['emg'])
            embedding_loss_e, vq_latents_e, _, perplexity_e = e_quantizer(e_latents)

            out = model(vq_latents_k) # [bs, 128, opt.dim_vq_latent]
            emb_loss = loss_fn(vq_latents_e, out)
            wandb.log({'train_emb_loss': emb_loss.item()})
            
            recon_emgs = vq_e_decoder(out) # [bs, seq_len, 6]
...
if __name__ == '__main__':
    ...
    scheduler = StepLR(optimizer, step_size=args.step_size, gamma=args.gamma) # after each batch before eval
    # scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=5, verbose=True, min_lr=1e-6) # after each epoch after eval
    loss_fn = nn.MSELoss()


    if opt.use_vq:
        wandb_project = 'Piano_EMG_NIPS25_ke_VQ_transformer'
        wandb_name = 'all_data_on_4090_128x256_seq2seq'
    else:
        wandb_project = 'Piano_EMG_NIPS25'
        wandb_name = 'all_data_on_4090_seq2seq'
