    ...
    with torch.no_grad():
        # with accelerator.autocast():
            for i, data in enumerate(tqdm(val_loader, desc="Validating", unit="batch")):
                ...
                # out_train = model(data['keystroke'], data['emg'])
                # loss_train = loss_fn(data['emg'], out_train)
                # total_val_loss_in_trainset += loss_train.item()
                # wandb.log({'total_val_loss_in_trainset': loss_train.item()})

                if i % opt.log_rate == 0:
                    # print(f"log_rate: {opt.log_rate}")
                    log_and_plot_batch(data['emg'], out, data['keystroke'], epoch, opt, "val")
                    # log_and_plot_batch(data['emg'], out_train, data['keystroke'], epoch, opt, "valInTrain")

    avg_val_loss = total_val_loss / len(val_loader)
    avg_val_loss_in_trainset = total_val_loss_in_trainset / len(val_loader)
    # if accelerator.is_main_process:
    wandb.log({'avg_val_loss': avg_val_loss})
    wandb.log({'avg_val_loss_in_trainset': avg_val_loss_in_trainset})
    scheduler.step()
...
def main():
    ...
    # model = EMGFormer()
    state_dict = torch.load('../../Piano_EMG_NIPS25_checkpoints/from_4090/epoch_30.pth')
    ...
    for epoch in range(opt.epoch):
        val_loss = train_and_val(model, train_loader, val_loader, optimizer, loss_fn, scheduler, opt, epoch, device)
        print(f"Epoch {epoch + 1}/{opt.epoch}, Val Loss: {val_loss:.4f}")
        logging.info(f"Epoch {epoch + 1}/{opt.epoch}, Val Loss: {val_loss:.4f}")
