...
class Residual(nn.Module):
  ...
  def __init__(self, fn, is_compressed=False, seq_len=1024, compressed_len=64):
    super().__init__()
    self.fn = fn
  ...
  #   x, mask_info = x_data
  #   x_resid, _ = self.fn((x, mask_info), *args, **kwargs)
  #   if self.is_compressed:
  #       x = nn.functional.interpolate(x.permute(0, 2, 1), size=self.compressed_len).permute(0, 2, 1)
  #       print(x.shape, x_resid.shape)
  #   return (x + x_resid, mask_info)
...
class MLP(nn.Module):
  ...
  def __init__(self, in_dim, out_dim, hidden_dim, is_compressed=False, seq_len=1024, compressed_len=64):
    super().__init__()
    self.is_compressed = is_compressed
    self.seq_len = seq_len
    self.compressed_len = compressed_len
  ...
  def forward(self, x_data):
    if type(x_data) is dict:
        ...
        if self.is_compressed:
            out = nn.functional.interpolate(out.permute(0, 2, 1), size=self.compressed_len).permute(0, 2, 1)
    ...
    if not is_compressed:
      ...
      for i in range(num_hidden_layers):
          blocks.extend([
              Residual(Norm(Attention(in_size, hidden_size,
                                      heads=num_attention_heads), hidden_size),
                                      is_compressed=True, seq_len=1024, compressed_len=64),
              Residual(Norm(MLP(hidden_size, hidden_size, intermediate_size, is_compressed=True, seq_len=1024, compressed_len=64),
                                hidden_size),
                                is_compressed=True, seq_len=1024, compressed_len=64)
          ])
